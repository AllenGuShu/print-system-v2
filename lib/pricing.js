...
function calcPrice(printType, pages, qty) {
  ...
  qty   = Number(qty)   || 0;
  if (!printType || pages <= 0 || qty <= 0) {
    return { sheets: 0, rate: 0, unit: 0, total: 0 };
  }
  const info = PRINT_TYPES[printType];
  if (!info) return { sheets: 0, rate: 0, unit: 0, total: 0 };

  const sheets = getSheets(printType, pages);

  if (info.fixedRate !== undefined) {
    return { sheets, rate: info.fixedRate, unit: info.fixedRate, total: Math.round(info.fixedRate * qty) };
  }
