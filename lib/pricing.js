...
const PRINT_TYPES = {
  ...
  'B5單張':                   { bind: 0,   cover: 0, fixedRate: 2.5 },
  ...
  '彩色印刷A4雙面':           { bind: 0,   cover: 0, fixedRate: 8   },

  // 頭條新聞獨立分類（A5/B5 × 黑白/彩色）
  '頭條新聞A5（黑白）':       { bind: 0,   cover: 0, fixedRate: 0.8  },
  '頭條新聞A5（彩色）':       { bind: 0,   cover: 0, fixedRate: 1.5  },
  '頭條新聞B5（黑白）':       { bind: 0,   cover: 0, fixedRate: 1.2  },
  '頭條新聞B5（彩色）':       { bind: 0,   cover: 0, fixedRate: 2.0  },
  '彩色文宣DM-A4':            { bind: 0,   cover: 0, fixedRate: 4.5 },
  '彩色文宣DM-B4':            { bind: 0,   cover: 0, fixedRate: 6.5 },
  '叫紙A4':                   { bind: 0,   cover: 0, fixedRate: 90  },
  '叫紙B4':                   { bind: 0,   cover: 0, fixedRate: 135 },
  '叫紙A3':                   { bind: 0,   cover: 0, fixedRate: 175 },
  '叫紙A5':                   { bind: 0,   cover: 0, fixedRate: 75  },
  '叫紙B5':                   { bind: 0,   cover: 0, fixedRate: 85  },
};
...
function getSheets(printType, pages) {
  ...
  // 雙面類型（含彩色雙面）：張數＝CEILING(頁,2)/2，優先判斷避免被「彩色」關鍵字誤吃
  if (printType.includes("雙面")) return Math.ceil(p / 2);
  const direct = ["B5單張", "彩色", "叫紙", "頭條"];
